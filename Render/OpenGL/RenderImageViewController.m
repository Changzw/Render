...
-(void)setUpConfig {
  ...
  /*3.配置视图创建的渲染缓存区.
   ...
   GLKViewDrawableDepthFormat24, 如果你要使用这个属性（一般用于3D游戏），
   你应该选择GLKViewDrawableDepthFormat16 或GLKViewDrawableDepthFormat24。
   使用GLKViewDrawableDepthFormat16 将消耗更少的资源
   */
  //3.配置视图创建的渲染缓存区.
...
-(void)setUpVertexData {
  //1.设置顶点数组(顶点[x,y,z]坐标,纹理[x,y]坐标)[内存]
  /*
  ...
  //(3).将顶点数组的数据【内存】copy到顶点缓存区中(GPU显存中)
  ...
  //打开顶点坐标数据通道
  glEnableVertexAttribArray(GLKVertexAttribPosition);// 打开通道开关
//读取（显示顶点->顶点着色器）
...
//绘制视图的内容
/* GLKView对象使其OpenGL ES上下文成为当前上下文，并将其framebuffer绑定为OpenGL ES呈现命令的目标。
 然后，委托方法应该绘制视图的内容。
 */
