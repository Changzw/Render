...
/*
 3种数据修饰类型
  ...
  attribute
  varying
 uniform -> 传递到 vertex，fragment 变量
 gluniform...
 
 vertex, fragment -> 常量
 
 uniform：
 uniform -> 客户端 -> 服务端（顶点shader) -> 服务端（偏远 shader）
 在顶点/片元着色器，进行一样的生命 -> 都传一样的
 ...
 attribute:
 特点：-> 客户端-> 顶点着色器(只能在顶点着色器里面用)
 用来修饰顶点纹理坐标颜色发现：坐标颜色相关
 glVertex...
 
 P.S 一定要打开开关
 attribute vec4 position; // 4维
 attribute vec4 color; // 4维
 attribute vec2 texCoord; // 2维
 */


/*
 不采样GLKBaseEffect(固定管线)，使用编译链接自定义的着色器（shader）。用简单的glsl语言来实现顶点、片元着色器，并图形进行简单的变换。
 思路：
 1.创建图层
 2.创建上下文
 3.清空缓存区
...
//2.设置上下文
-(void)setupContext {
  //1.指定OpenGL ES 渲染API版本，我们使用2.0.创建图形上下文
